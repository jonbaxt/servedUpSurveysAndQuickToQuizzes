export default function MultChoice5Answers(survey3MultipleChoiceQuestions, questionNumber){
    ...
    let surveyMultChoiceQuesAnswersOption4 = survey3MultipleChoiceQuestions.filter(el => el.ques_num === questionNumber).map(el => {
        let choice4 = 0;
        if (el.takers_answer === survey3MulChQuesOneAnswers[3]) {
            choice4++
        }
        return (choice4)
    }).reduce((prev, curr) => prev + curr)
    let surveyMultChoiceQuesAnswersOption5 = survey3MultipleChoiceQuestions.filter(el => el.ques_num === questionNumber).map(el => {
        let choice5 = 0;
        if (el.takers_answer === survey3MulChQuesOneAnswers[4]) {
            choice5++
        }
        return (choice5)
    }).reduce((prev, curr) => prev + curr)
    ...
    let table1MultChoiceQuesResults = {
        ...
        datasets: [{
            ...
            data: [surveyMultChoiceQuesAnswersOption1, surveyMultChoiceQuesAnswersOption2, surveyMultChoiceQuesAnswersOption3, surveyMultChoiceQuesAnswersOption4, surveyMultChoiceQuesAnswersOption5]
        }]
    };
